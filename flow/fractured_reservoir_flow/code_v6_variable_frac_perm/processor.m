...
% ---------------------------------------------------------------------- %
    ...
    frac.nodeOnBothEnds = intersect(frac.map(:,1),frac.map(:,2));    % nodes both on p1 and p2
    frac.nodeOnJustPnt1 = setdiff(frac.map(:,1),frac.map(:,2));      % nodes just on p1, not on p2
    frac.nodeOnJustPnt2 = setdiff(frac.map(:,2),frac.map(:,1));      % nodes just on p2, not on p1
    
    % determining number of unknowns
    
    Nw = size(bhc.well,1);              % number of hole conditions applied node
    Nd = size(frac.nodeOnBothEnds,1);   % number of double neighbor nodes
    ...
    frac.pnt1(:,1) = frac.nodes(frac.map(:,1),1);
    frac.pnt1(:,2) = frac.nodes(frac.map(:,1),2);
    frac.pnt2(:,1) = frac.nodes(frac.map(:,2),1);
    frac.pnt2(:,2) = frac.nodes(frac.map(:,2),2);
    ...
    frac.length = sqrt(sum((frac.pnt2-frac.pnt1).^2,2));
    
    frac.area = res.zlength*frac.length;
    
    frac.center(:,1) = (frac.pnt1(:,1)+frac.pnt2(:,1))/2;
    frac.center(:,2) = (frac.pnt1(:,2)+frac.pnt2(:,2))/2;
    ...
    FC = frac.permeability.*frac.width;

...
class IMPES():
	...
	def solve(self):
		...
		for time in self.time_array[:2]:
			...
			Sw_xm[Uxm] = self.Sw[cxm_0][Uxm]
			Sw_xp[Uxp] = self.Sw[cxp_0][Uxp]
			Sw_ym[Uym] = self.Sw[cym_0][Uym]
			Sw_yp[Uyp] = self.Sw[cyp_0][Uyp]
			Sw_zm[Uzm] = self.Sw[czm_0][Uzm]
			Sw_zp[Uzp] = self.Sw[czp_0][Uzp]

			Sw_xm[~Uxm] = self.Sw[cxm_m][~Uxm]
			Sw_xp[~Uxp] = self.Sw[cxp_p][~Uxp]
			Sw_ym[~Uym] = self.Sw[cym_m][~Uym]
			Sw_yp[~Uyp] = self.Sw[cyp_p][~Uyp]
			Sw_zm[~Uzm] = self.Sw[czm_m][~Uzm]
			Sw_zp[~Uzp] = self.Sw[czp_p][~Uzp]

			krw_xm,kro_xm = self.rp.oil_water(Sw=Sw_xm)
			krw_ym,kro_ym = self.rp.oil_water(Sw=Sw_ym)
			krw_zm,kro_zm = self.rp.oil_water(Sw=Sw_zm)
